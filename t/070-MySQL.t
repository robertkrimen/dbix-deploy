...
my $setup = sub { $deploy->generate($deploy->_script->{setup}->[0]->command->arguments->{sql}) };
is(${ $setup->() }, <<_END_);
...
my $teardown = sub { $deploy->generate($deploy->_script->{teardown}->[0]->command->arguments->{sql}) };
is(${ $teardown->() }, <<_END_);
...
my $create = sub { $deploy->generate($deploy->_script->{create}->[0]->command->arguments->{sql}) };
is(${ $create->() }, <<_END_);
